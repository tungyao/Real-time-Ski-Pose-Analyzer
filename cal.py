...
def smooth_landmarks(prev, curr, alpha=0.7):
    ...
    for p, c in zip(prev, curr):
        smoothed.append([
            alpha * p[0] + (1 - alpha) * c[0],
            alpha * p[1] + (1 - alpha) * c[1],
            alpha * p[2] + (1 - alpha) * c[2],
        ])
...
class VideoThread(QThread):
    ...
    model_complexity=2,                # 模型精度提升
    enable_segmentation=True,          # 可选：人体分割
    smooth_landmarks=True,             # 时序平滑
    min_detection_confidence=0.6,      # 初始检测置信度
    min_tracking_confidence=0.8        # 跟踪置信度
            )
